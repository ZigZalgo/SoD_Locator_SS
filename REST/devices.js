...
exports.updateOrientation= function(req, res){
    ...
    for(var key in locator.devices){
        if(locator.devices.hasOwnProperty(key)&&locator.devices[key].uniqueDeviceID==req.params.id){
            locator.devices[key].orientation.yaw = Math.round(req.params.orientation*100)/100;
            console.log('udpate device:'+locator.devices[key].uniqueDeviceID+' orientation to: '+JSON.stringify(locator.devices[key].orientation));
        }
    }
    locator.refreshStationarylayer();
    res.status(200);
